...
function CardTest({ image, info, handleClickDetail, handleClickStartTest }) {
  return (
    <div className="flex justify-between items-center rounded-md shadow-lg shadow-gray-400
    bg-rose-200 max-width-full h-24 mx-5 my-5 hover:-translate-y-1 transition duration-300 ease-in-out">
      <div className="image-time flex text-slate-600">
        <img
          className="rounded size-16 border-2 border-gray-300 mx-4"
          src={image}
          alt="ảnh bài thi "
        />
      ...
      <div className="action mr-4 space-x-5">
        <Button type={"button"} onClick={() => handleClickDetail(info.name)}>
          Chi tiết
        </Button>
        <Button type={"button"} onClick={() => handleClickStartTest(info.id)}>
          Vào thi
        </Button>
      </div>
    </div>
