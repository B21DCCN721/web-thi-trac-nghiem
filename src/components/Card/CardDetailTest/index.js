...
function CardDetailTest({info}) {
    ...
    return ( 
        <div className="mx-5 mt-5 rounded-md shadow-lg *:m-5">
            ...
            <p><span className="mr-3"><FontAwesomeIcon icon={faClock} /></span>Thời gian làm tạo:{info.time}</p>
            <p><span className="mr-3"><FontAwesomeIcon icon={faCircleQuestion} /></span>Số câu hỏi:{info.quantity}</p>
            <p><span className="mr-3"><FontAwesomeIcon icon={faUser} /></span>Số lượt làm:{info.turn}</p>
            <div>
                <select 
                    value={selectedTime} 
